...
| [`gplay-release-flow`](skills/gplay-release-flow/SKILL.md) | Uploading a build (AAB, or a legacy APK), cutting a release, promoting a build up the track ladder (internal → alpha → beta → production), running or steering a staged production rollout (rollout / halt / resume / complete), or inspecting which releases sit on a track. |
...
| [`gplay-reviews`](skills/gplay-reviews/SKILL.md) | Triaging recent reviews, filtering by star rating, replying to a user, or bulk-answering reviews from a TSV in CI (the API exposes the last 7 days), or reading the full review history from the monthly GCS CSV reports with `reviews history`. |
